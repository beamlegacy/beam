    ...
    func buildStringFrom(nodes: [ElementNode]) -> NSAttributedString {
        ...
        for node in nodes {
            ...
            if nodes.count > 1 {
                ...
                strNodes.append(node.text.buildAttributedString(fontSize: node.fontSize, cursorPosition: node.cursorPosition, elementKind: node.elementKind, mouseInteraction: nil, markedRange: rootNode.markedTextRange))
