...
function beam_getSelectedHtml() {
    ...
    if (selection) {
        for (let i = 0; i < selection.rangeCount; i++) {
            const cloned = selection.getRangeAt(i).cloneContents();
...
let beam_currentSelectedText = "";
let beam_currentSelectedHtml = "";

function beam_textSelected() {
    window.webkit.messageHandlers.beam_textSelected.postMessage({
        selectedText: beam_currentSelectedText,
        selectedHtml: beam_currentSelectedHtml.innerHTML
    });
}
...
if (video) {
    video.addEventListener('durationchange', (_event) => {
        console.log('Video duration changed ' + video.duration.toString());
    });

    video.addEventListener('timeupdate', (_event) => {
        console.log('Video progress ' + video.currentTime.toString() + ' / ' + video.duration.toString());
    });
}
