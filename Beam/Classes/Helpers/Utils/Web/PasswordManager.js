function beam_isTextField(element) {
    ...
    if (elementType !== 'text' && elementType !== 'password' && elementType !== 'email' && elementType !== '' && elementType !== null) {
        return false;
    }
    return true;
}

function beam_getTextFieldsInDocument(doc, frame) {
    var textFields = [];
    for (tagName of ['input', 'select', 'textarea']) {
        let inputElements = document.getElementsByTagName(tagName);
        for (var e = 0; e < inputElements.length; e++) {
...
function beam_sendTextFields() {
    var textFields = beam_getTextFieldsInDocument(document, null);
