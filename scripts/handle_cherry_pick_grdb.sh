...
LAST_DEVELOP_COMMIT=$(curl -sS -H "PRIVATE-TOKEN: $GITLAB_API_ACCESS_TOKEN" "${GITLAB_API_URL}/projects/$CI_PROJECT_ID/repository/branches/develop-grdb" | \
...
# Do not create cherry pick for the first build of release branch from develop-grdb
if [ "$LAST_DEVELOP_COMMIT" != "$RELEASE_COMMIT" ]; then
    
    # create branch for the cherry pick
    curl -sS -X POST -H "PRIVATE-TOKEN: $GITLAB_API_ACCESS_TOKEN" "${GITLAB_API_URL}/projects/$CI_PROJECT_ID/repository/branches?ref=develop-grdb&branch=cherry-pick-grdb/$CI_COMMIT_SHORT_SHA"

    # cherry pick to branch
    curl -sS -X POST -H "PRIVATE-TOKEN: $GITLAB_API_ACCESS_TOKEN" "${GITLAB_API_URL}/projects/$CI_PROJECT_ID/repository/commits/$CI_COMMIT_SHA/cherry_pick?branch=cherry-pick-grdb/$CI_COMMIT_SHORT_SHA"
    ...
    MR_IID=$(curl -sS -X POST -H "PRIVATE-TOKEN: $GITLAB_API_ACCESS_TOKEN" "${GITLAB_API_URL}/projects/$CI_PROJECT_ID/merge_requests?squash=true&source_branch=cherry-pick-grdb/$CI_COMMIT_SHORT_SHA&target_branch=develop-grdb&remove_source_branch=true&title=Cherry%20Pick%20$MR_TITLE%20into%20develop%20GRDB&approvals_before_merge=0" | \
    	jq '.iid')
